...
def draw_vertical_lines_on_image(input_image_path, output_image_path, line_distance):
    ...
    '''
    # Sum histogram values in a ±3 pixel range for peak detection
    countsX = np.convolve(countsX, np.ones(3), mode='same')  # A window size of 7 covers ±3 pixels
    # Find peaks in the histogram
    peaks, _ = find_peaks(countsX, height=50, distance=50)
    ...
    if len(peaks) >= 2:
        ...
        plt.axvline(bin_centers[peaks ], color='r', linestyle='--', label='Peak 1')
...
# Parameters
input_image_path = r'D:\My Files\HiWi IFPT\hiwiifpt\Images\40mm\Screenshot_test_40.png'  # Path to the input image
...
thickness = 1  # Thickness of the edges of the edge image
dot_radius = 2  # Radius of the red dots at intersection
